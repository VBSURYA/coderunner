...
import { FileItem, TerminalOutput, EditorSettings, DEFAULT_FILES, RunResult, LANGUAGE_METADATA } from './types';
...
export default function App() {
  ...
  const handleCreateFile = (name: string, language: string) => {
    const defaultTemplate = LANGUAGE_METADATA[language]?.defaultTemplate || '// Write your code here';
  ...
  const assembleWebWorkspace = (activeFile: FileItem) => {
    ...
    const jsFile = files.find(f => f.language === 'javascript' && f.id !== activeFile.id);
  ...
  const handleRunCode = async () => {
    ...
    const commandToLog = activeFile.language === 'javascript' ? 'node' : activeFile.language;
    setTerminalLogs(prev => [...prev, {
      type: 'system',
  ...
  return (
    <div id="workspace-layout-root" className="flex flex-col h-screen w-screen bg-[#1e1e1e] text-[#cccccc] overflow-hidden font-sans antialiased">
      ...
      <header className="flex items-center justify-between px-4 h-9 bg-[#323233] border-b border-[#1e1e1e] shrink-0 select-none text-[#cccccc]">
        <div className="flex items-center gap-4 text-xs font-normal">
          ...
          <div className="flex items-center gap-1.5 font-semibold text-white">
            ...
            <span className="hidden sm:inline text-[11px] uppercase tracking-wider text-gray-300">File</span>
          </div>
          <span className="cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">Edit</span>
          <span className="hidden md:inline cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">Selection</span>
          <span className="cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">View</span>
          <span className="hidden md:inline cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">Go</span>
          <span className="cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">Run</span>
          <span className="hidden sm:inline cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">Terminal</span>
          <span className="cursor-pointer hover:bg-[#474749] px-1.5 py-0.5 rounded transition">Help</span>
        </div>

        {/* Workspace App Title */}
        <div className="flex-1 text-center text-[11px] text-[#858585] truncate font-mono px-4">
          {files.find(f => f.id === activeFileId)?.name || 'index.js'} — CodeRunner Workspace (Vite + NodeVM)
        </div>

        {/* Window controls mimicking desktop app */}
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 bg-[#4f4f4f] hover:bg-[#ff5f56] rounded-full transition cursor-pointer" title="Close" />
          <div className="w-2.5 h-2.5 bg-[#4f4f4f] hover:bg-[#ffbd2e] rounded-full transition cursor-pointer" title="Minimize" />
          <div className="w-2.5 h-2.5 bg-[#4f4f4f] hover:bg-[#27c93f] rounded-full transition cursor-pointer" title="Maximize" />
        </div>
      </header>
      ...
      <footer className="h-6 bg-[#007acc] text-white flex items-center justify-between px-3 text-[11px] shrink-0 select-none">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-0.5"><span className="text-white">⊗</span> 0</span>
            <span className="flex items-center gap-0.5"><span className="text-white">⚠</span> {runResult?.compileError ? '1' : '0'}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="hidden sm:inline hover:bg-[#0062a3] px-1.5 py-0.5 rounded cursor-pointer">UTF-8</span>
